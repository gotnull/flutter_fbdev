...
void main() {
  ...
  test('analog stick axis self-calibrates to a normalized position', () async {
    HandheldInput.instance.start();
    // Left stick X (RG34XXSP ABS 2): sweep the range, then read the centre.
    await send('abs:2:0'); // min
    await send('abs:2:1000'); // max -> establishes the range
    await send('abs:2:500'); // halfway -> ~0
    expect(HandheldInput.instance.sticks.value.leftX, closeTo(0, 0.01));
    await send('abs:2:1000'); // full right -> +1
    expect(HandheldInput.instance.sticks.value.leftX, closeTo(1, 0.01));
    // Stick axes must not leak into button events.
    final events = capture();
    await send('abs:4:250');
    expect(events, isEmpty);
  });
}
