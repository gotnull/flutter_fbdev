...
- **analog sticks** (`HandheldInput.sticks`, self-calibrating per axis);
- **looped audio** (`FbdevAudio.play`) - the embedder decodes a bundled Ogg
  Vorbis file (stb_vorbis) and plays it via ALSA, so you get sound on boards
  where the usual Flutter audio plugins don't run;
...
> Built while porting a touch-based Flutter game to an Anbernic RG34XXSP. It
> renders, the colours are right, and the d-pad and A/B work.
...
[`example/`](example) is a cosy showcase that proves every part:
